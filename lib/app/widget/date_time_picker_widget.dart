...
class _DateTimePickerWidgetState extends State<DateTimePickerWidget> {
  ...
  void didChangeDependencies() {
    if (widget.isTimePicked &&
      ...
      _selectedDateTime =
          "${DateFormat("dd/MM/yyyy").format(widget.defaultDate!)}  ${widget.defaultTime!.format(context)}";
        ...
        if (timePicked != null) {
          setState(() {
            datePicked = datePicked!.add(
                Duration(hours: timePicked.hour, minutes: timePicked.minute));
            _selectedDateTime =
                "${DateFormat("dd/MM/yyyy").format(datePicked!)}  ${timePicked.format(context)}";
            widget.onChanged(
                _selectedDateTime, datePicked!.millisecondsSinceEpoch ~/ 1000);
          });
        }
      } else {
        setState(() {
